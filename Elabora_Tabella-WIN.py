...
for c in lnome:
    ...
    if WIN[c]['virtuale'] == 'SI':
        ...
        if w > 6:           # ----------------------- Il numero di macchine virtuali è tale da suggerire di licenziare i core fisici degli host.
            ...
            WIN[c]['standard_lic_number'] = cm.pr["Minimo_lic_Win_vm"] if z < int(cm.pr["Minimo_lic_Win_vm"]) else z
            WIN[c]['datacenter_lic_number'] = 0            
    else:
        if WIN[c]['nome_edizione'] == 'datacenter':
            WIN[c]['standard_lic_number'] = 0
            WIN[c]['datacenter_lic_number'] = cm.pr["Minimo_lic_Win_fs"] if z < int(cm.pr["Minimo_lic_Win_fs"]) else z
        else:
            WIN[c]['standard_lic_number'] = cm.pr["Minimo_lic_Win_fs"] if z < int(cm.pr["Minimo_lic_Win_fs"]) else z
            WIN[c]['datacenter_lic_number'] = 0            
