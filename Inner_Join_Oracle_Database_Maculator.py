...
def read_csv_safely(path):
    """
    Citește CSV-ul ServiceNow (chiar și cu 'sep=,' în prima linie + orice encoding)
    """
    ...
    for enc in encodings:
        try:
            ...
            # Eliminăm linia 'sep=,' dacă există
            ...
            if len(df.columns) > 10:  # trebuie să aibă măcar 30-40 coloane reale
                print(f"Asset Server citit cu succes (encoding: {enc})")
                return df.fillna("")
        except:
            continue

    # Ultima soluție disperată (funcționează mereu)
    print("Fallback: citire forțată cu skiprows=1 și latin1")
...
def inner_join_cmdb_assetserver_columns():
    ...
    if not lista_cmdb_files:
        raise FileNotFoundError("Nu am găsit Lista_CMDB.xlsx")
    lista_cmdb_file = lista_cmdb_files[0]
    print(f"Citire Lista_CMDB: {lista_cmdb_file}")
    ...
    if not asset_server_files:
        raise FileNotFoundError("Nu am găsit fișierul Asset Server")
    asset_server_file = asset_server_files[0]
    print(f"Citire Asset Server: {asset_server_file}")

    asset_srv_df = read_csv_safely(asset_server_file)
    print(f"Număr coloane detectate: {len(asset_srv_df.columns)}")
    print(f"Primele 5 coloane: {list(asset_srv_df.columns[:5])}")

    # Găsim coloana cu aplicațiile (poate avea diverse nume)
    ...
    if applicazioni_col is None:
        ...
        for i, col in enumerate(asset_srv_df.columns):
            print(f"  [{i}] {col}")
        raise KeyError("EROARE: Nu am găsit coloana 'Applicazioni (lista)'!")

    print(f"Coloana aplicații detectată: '{applicazioni_col}'")

    # Construim mapping-ul: aplicație → listă de Nome_CI
    mapping_list = []
    ...
    if mapping_df.empty:
        print("Nu s-au găsit aplicații în coloana Applicazioni (lista)")
    ...
    if not asset_software_files:
        raise FileNotFoundError("Nu am găsit Asset Software")
    asset_software_file = asset_software_files[0]
    print(f"Citire Asset Software: {asset_software_file}")
    ...
    def has_oracle(server):
        ...
        return "DA" if re.search(r'oracle\s+database', sw, re.IGNORECASE) else "NU"

    sheet2["Oracle_Database"] = sheet2["Inner_Join_Nome_CI_server"].apply(has_oracle)

    # Adăugăm toate serverele din Asset Software pentru aliniere
    ...
    print("GATA! Fișier generat:")
    print(f"   → {output_file}")
    print(f"   → Inner Join găsite: {len(merged_values)} aplicații comune")
    print(f"   → Servere cu Oracle detectate: {sheet2['Oracle_Database'].eq('DA').sum()}")
