...
for linea in Linee:
    ...
    if group_type == 'STD':
        GROUP_STD.append(group_name)
    else:
        GROUP_PRO.append(group_name)
...
print("\n\n+-------------------------------------------------------+\n")
print(f"Per i gruppi indicati in 'List gruppi office PROPlus FLEXERA' risultano {n} utenti")

n = len(USER_GROUP_STD)
print("\n\n+-------------------------------------------------------+\n")
print(f"Per i gruppi indicati in 'List gruppi office Standard FLEXERAa' risultano {n} utenti")

# --- Scrierea fișierului 'User_list_pro.csv'
# --- Scrittura del file 'User_list_pro.csv'
#
y = [cm.out_path, cm.pr["FLEXERA_OUT_USER_PRO"]]
...
#
y = [cm.out_path, cm.pr["FLEXERA_OUT_USER_STD"]]
output_file = cm.dr.join(y)
