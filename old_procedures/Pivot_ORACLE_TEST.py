...
# Pattern fisier ORACLE
cmdb_pattern = parametri["OUT_Oracle_TAB"]
...
if file_path is None:
    raise FileNotFoundError("Nu a fost gasit fisierul Tabella_Oracle in output conform patternului!")
...
output_path = os.path.join(output_dir_full, "pivot_ORACLE_results.txt")
...
"""
#  QUERY NOU pentru ORACLE

SELECT 
    Edizione,
    contratto,
    SUM(licenze) AS Somma_licenze
FROM tabella_oracle
WHERE 
    ambiente NOT IN ('DR;', 'TEST;', '-')
    AND cliente NOT IN ('BANCA MEDIOLANUM', 'BANCA UBAE', 'TEST', 'Ribes')
    AND Edizione <> 'Oracle Database - Express Edition'
...
tabela1 = (
    df["Edizione"]!='Oracle Database - Express Edition'
)
