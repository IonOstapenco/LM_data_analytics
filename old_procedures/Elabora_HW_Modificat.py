...
Hardware_field = {
    ...
    "vm_hw_version": {"VM": 8},         
    "n_cpu": {"BGFX": 7, "CMDB_Citrix": 5},  # CORRETTO: era 6, ora 5                
    "n_core": {"VM": 5, "BGFX": 6, "CMDB_Citrix": 5},  # CORRETTO: era 6, ora 5                            
    "pvu_per_core": {"BGFX": 21},             
    "valore_pvu_modificato": {"BGFX": 22},      
    "valore_pvu_predefinito": {"BGFX": 23},     
    "fattore_core_oracle": {"BGFX": 24},       
    "powerstate": {"VM": 1},                
    "vcenter": {"CMDB_Citrix": 10, "VM": 16, "Cluster": 13},  # CORRETTO: era 11, ora 10                 
    "vcenter_cores": {"VCenter": 2},        
    "vcenter_hosts": {"VCenter": 3},        
    "vcenter_dominio": {"VCenter": 4},      
    "vcenter_cluster": {"VCenter": 1},      
    "cpu_model": {"BGFX": 17, "CMDB_Citrix": 7},  # CORRETTO: era 8, ora 7              
...
CMDB_Citrix = {}
# AGGIORNATO per corrispondere alla nuova struttura
CMDB_field = {
    "Nome CI": 0, 
    "OS": 1, 
    "DNS": 2, 
...
DISS_field = {
    "server": 0,
    "datadismissione": 1  # CORRETTO: era 2, ora 1
}
...
# Questi sono i nomi del server elencati nella lista unita tra CMDB e Citrix
lnome = list(CMDB_Citrix.keys())
# Questi sono i nomi dei campi che dobbiamo compilare per Tabella HW
lfield = list(Hardware_field.keys())

print("\nElaborazione Hardware in corso...")
i = 0
for k in lnome:  # 'k' contiene il nome del server, dall'elenco CMDB_Citrix, in fase di elaborazione
    ...
    for f in lfield:  # 'f' contiene il nome del campo da compilare in Tabella HW
        # Questo è l'elenco delle priorità: da quale origine prendere il valore per primo
        p = list(Hardware_field[f].keys())
        
        for a in p:  # 'a' contiene il nome dell'origine da cui prendere il valore
            match a:
                case "CMDB_Citrix":
                    idx = Hardware_field[f][a]
                    # PROTEZIONE CONTRO IndexError
                    if idx < len(CMDB_Citrix[k]):
                        Hardware[k][f] = CMDB_Citrix[k][idx]
                        break
                    else:
                        Hardware[k][f] = "-"
                ...
                case "Cluster":
                    if CMDB_Citrix[k][CMDB_field["Is Virtual"]] == "NO":
                        ...
                        try:
                            cluster_name = CMDB_Citrix[k][CMDB_field["VM_Cluster"]]
