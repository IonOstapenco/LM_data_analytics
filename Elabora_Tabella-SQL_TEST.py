...
SQL_Field = {
    ...
    "contratto": 38 # --> adaugat recent, in 04_2026
}
SQL_read = 11 # ar tebui sa fie 12, dar vad ca lucreaza si asa
...
# si aici s-au facut modificari recente
for k in Hardware:
    ...
    if (sysop.find("sql") > -1) or (k in Software):
        ...
        for c in SQL_Field:
            if c == "edizione":
                # vine din Software, nu din Hardware
                if k in Software: # --> bucata asta am copiat-o de jos unde era
                    SQL[k].append(Software[k])
                else:
                    SQL[k].append("-")
                continue # in loc de break 
