...
$outputPath = Join-Path $basePath "gruppi_TEST" # in mapa Report_202606 se va crea mapa all_in_one1
...
# a doua varianta pentru citirea AD_Users_Results_attivi
#$adUsersFile = Join-Path $basePath "output-1\AD_Users_Results_attivi.csv"

# ================== PATH ==================
#$basePath   = "C:\License_Management\Report_202605" # fisier de baza   --> Report_dir
#$outputPath = "C:\License_Management\Report_202605\all_in_one" # mapa de output ---> "Output_dir": "OutPut",
$outFile    = Join-Path $outputPath "Licenze Office STD (solo_PRD_EXCEPTIE _ctx_047_tta_ads)_TEST.csv"
$outFile2   = Join-Path $outputPath "Licenze Office STD (solo_PRD_EXCEPTIE _ctx_047_tta_ads)_Unique Names_TEST.csv"
$outFile3   = Join-Path $outputPath "Licenze Office STD_(solo_PRD_EXCEPTIE _ctx_047_tta_ads)_Unique SamAccountNames_TEST.csv"

# adaugat
$outFile4   = Join-Path $outputPath "Licenze Office STD_(solo_PRD_EXCEPTIE _ctx_047_tta_ads)_Unique Names_Banche_TEST.csv"

#$adUsersFile = "C:\License_Management\Report_202605\output-1\AD_Users_Results_attivi.csv"
...
###
#=============
# scoaterea din coloana Name celea denumiri, care sunt deja in SamAccountName
...
$cleanResultsName |
    Export-Csv $outFile2 -NoTypeInformation -Delimiter ";"

$cleanResultsSamAccountName |
    Export-Csv $outFile3 -NoTypeInformation -Delimiter ";"  


$cleanResultsWithBanche |
    Export-Csv $outFile4 -NoTypeInformation -Delimiter ";"    


Write-Host " Gata! Fisier generat: $outFile"
Write-Host " Gata! Fisier generat: $outFile2"
Write-Host " Gata! Fisier generat: $outFile3"
Write-Host " Gata! Fisier generat: $outFile4"
'''
...
# =====================================================
# CREARE EXCEL DIN CSV-URILE GENERATE
# =====================================================
...
csv1 = os.path.join(
    outputPath,
    "Licenze Office STD (solo_PRD_EXCEPTIE _ctx_047_tta_ads)_TEST.csv"
)

csv2 = os.path.join(
    outputPath,
    "Licenze Office STD (solo_PRD_EXCEPTIE _ctx_047_tta_ads)_Unique Names_TEST.csv"
)

csv3 = os.path.join(
    outputPath,
    "Licenze Office STD_(solo_PRD_EXCEPTIE _ctx_047_tta_ads)_Unique SamAccountNames_TEST.csv"
)

csv4 = os.path.join(
    outputPath,
    "Licenze Office STD_(solo_PRD_EXCEPTIE _ctx_047_tta_ads)_Unique Names_Banche_TEST.csv"
)
...
excelFile = os.path.join(
    outputPath,
    "Licenze_Office_STD_FINAL_INNER JOIN AD_RESULTS_ATTIVI AND UNIQUE NAMES -- MAtch AD.xlsx"
)
...
unique_names_df = pd.read_csv(csv2, sep=";")
...
with pd.ExcelWriter(excelFile, engine="openpyxl") as writer:
    ...
    pd.read_csv(csv1, sep=";").to_excel(
        writer,
        sheet_name="Dettaglio",
        index=False
    )

    pd.read_csv(csv2, sep=";").to_excel(
        writer,
        sheet_name="Unique Names",
        index=False
    )

    pd.read_csv(csv3, sep=";").to_excel(
        writer,
        sheet_name="Unique SAM",
        index=False
    )

    pd.read_csv(csv4, sep=";").to_excel(
        writer,
