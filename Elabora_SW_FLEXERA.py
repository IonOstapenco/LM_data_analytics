...
# class din Flexera --> adaugast pe 11/5/2026 (copiat si modificat de mai sus)
class c_FLEXERASW:
    def __init__(self, nume):
        self.nome = nume
        self.sw = []
...
# adaugam dictionar FLEXERA
FLEXERASW = {} # --> adaugat 11/5/2026
...
##################################33333
# procesare FLEXERA --> adaugat pe data de 11/5/2026

# ───────────────────────────────────────────────
# 5. Procesare FLEXERA SW
# ───────────────────────────────────────────────
print("\nElaborare FLEXERASW...")

cm.files = []
file_pattern = cm.pr["FLEXERA_SOFTWARE"]
...
if not cm.files:
    print("→ Niciun fișier FLEXERA_SW găsit")
else:
    file_path = sorted(cm.files, reverse=True)[0]
    print(f"→ Procesez: {file_path}")
    ...
    with open(file_path, "r", encoding="utf-8-sig", errors="replace") as f:
        ...
        reader.fieldnames = [
            norm(h) for h in reader.fieldnames
            if h is not None
        ]

        print(
            "Header FLEXERA:",
            ", ".join(reader.fieldnames[:12]) + "..."
        )
        ...
        for row in reader:
            ...
            versione = norm(
                clean.get("versionName", ""),
                lower=True
            )

            edition = norm(
                clean.get("editionName", ""),
                lower=True
            )
            ...
            # filtre vendor
            if publisher and cm.cerca_elemento(
                cm.pr["Vendor"],
                publisher,
                -1,
                1,
            ...
            sw_line = [
                ...
                nome_prodotto,      # nome componente
                versione,           # versione_componente
                alias,              # versione dettagliata componente
                "-",                # path
                nome_prodotto,      # nome prodotto
                versione,           # versione_prodotto
                "-",                # metrica
                edition,            # nome_edizione
                versione            # versione_edizione
            ]

            if computer not in FLEXERASW:
                FLEXERASW[computer] = c_FLEXERASW(computer)

            FLEXERASW[computer].software(sw_line)

            kept_count += 1

        print(
            f"   Total rânduri: {row_count:,} "
            f"→ păstrate: {kept_count:,}"
        )
...
for nume in list(Software.keys()):
    ...
    # adaugat 14/5/2026
    if nume in FLEXERASW:
        for asset in FLEXERASW[nume].sw:
            ed_n, ed_v = calcola_edizione(asset.datisw)
            asset.datisw[8] = ed_n or asset(ed_n) 
            Software[nume].software(asset.datisw[:])


# adaugat pe data de 14-05/2026
#DIAGNOSTIC
print(f"Servere cu FLEXERA SW : {len(FLEXERASW):>6}")
comune_flexera = set(Hardware.keys()) & set(FLEXERASW.keys())

print(f"Servere comune Hardware ↔ FLEXERA : {len(comune_flexera):>6}")

if comune_flexera:
    print("Exemple comune FLEXERA:", list(comune_flexera)[:3])
...
# 6. Scriere finală
# ───────────────────────────────────────────────
output_file = cm.dr.join([cm.out_path, cm.pr["OUT_Software_TAB_FLEXERA"]])
print("\n=== DIAGNOSTIC ===")
...
# inscriere in mapa Output_FLEXERA
output_flexera_dir = cm.dr.join([
    cm.start_path,
    cm.pr["Output_dir_FLEXERA"]
])

cm.check_outdir(output_flexera_dir)

output_file_flexera = cm.dr.join([
    output_flexera_dir,
    cm.pr["OUT_Software_TAB_FLEXERA"]
])

with open(output_file_flexera, "w", encoding="utf-8") as f:
...
print(f"\nFile FLEXERA scritto: {output_file_flexera}")
